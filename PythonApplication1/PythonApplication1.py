...
def draw_prompt(screen, font):
    show_e = False
    if current_level == "world" and check_house_entry(player_pos) is not None:
        show_e = True
...
def main():
    ...
    global current_level, current_house_index
    ...
    while True:
        ...
        if current_level != "world":
            indoor_size = PLAYER_SIZE * 4 # or 3 if you want it even bigger
            frame = pygame.transform.scale(frames[current_direction][player_frame_index if moving else 0],
        ...
        # House exit
        if current_level == "house" and keys[pygame.K_e]:
